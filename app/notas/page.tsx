...
import { FiPlus, FiFileText, FiTrash2, FiClock, FiMenu, FiEdit, FiSave, FiX, FiSearch } from "react-icons/fi";
...
export default function NotasPage() {
  ...
  return (
    <div className="p-4 lg:p-8 max-w-[1600px] mx-auto animate-fade-in">
      ...
      {notas.filter(n => 
        ...
        <div className="text-center py-24 bg-gray-50 dark:bg-gray-900 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-[40px]">
          ...
          <p className="text-gray-400 dark:text-gray-500 font-bold uppercase text-xs tracking-widest">
            {searchTerm ? "No se encontraron resultados" : "No hay notas guardadas"}
          </p>
        </div>
      ) : (
          ...
          {notas.filter(n => 
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className={`p-4 rounded-2xl text-white transition-all transform group-hover:rotate-6 ${
                  nota.prioridad === NotaPrioridad.Alta ? 'bg-red-500 shadow-red-200' :
                  nota.prioridad === NotaPrioridad.Media ? 'bg-yellow-500 shadow-yellow-200' :
                 ...
                 <div className="flex flex-col items-end gap-2">
                    <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${
                      nota.prioridad === NotaPrioridad.Alta ? 'bg-red-100 text-red-600' :
                      nota.prioridad === NotaPrioridad.Media ? 'bg-yellow-100 text-yellow-600' :
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNuevaNota({...nuevaNota, prioridad: Number(e.target.value)})}
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-400 mb-2 uppercase tracking-wider">Categoría</label>
                  <select
                    value={nuevaNota.categoria}
                    onChange={(e) => setNuevaNota({...nuevaNota, categoria: Number(e.target.value)})}
                    className="w-full bg-gray-50 dark:bg-gray-800 border-2 border-gray-100 dark:border-gray-700 rounded-xl p-3 font-bold outline-none focus:border-black dark:focus:border-white transition-all text-sm"
                  >
                    ...
                    <option value={TaskCategory.Documentacion}>DOCUMENTACIÓN</option>
                    <option value={TaskCategory.Mantenimiento}>MANTENIMIENTO</option>
                  </select>
                </div>
              </div>

              <textarea
                placeholder={error && !nuevaNota.contenido ? "¡La nota no puede estar vacía!" : "Contenido de la nota (Soporta Markdown)..."}
      ...
      {/* Modal Detalle Nota */}
      {selectedNota && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white dark:bg-gray-900 w-full max-w-3xl max-h-[90vh] rounded-[24px] md:rounded-[32px] p-6 md:p-12 shadow-2xl dark:shadow-none relative animate-scale-up flex flex-col">
            <button 
              onClick={(e) => { e.stopPropagation(); setSelectedNota(null); setIsEditing(false); }}
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-6 md:mb-8">
               <div className={`p-4 rounded-2xl text-white shadow-lg ${
                  selectedNota.prioridad === NotaPrioridad.Alta ? 'bg-red-500 shadow-red-200' :
                  selectedNota.prioridad === NotaPrioridad.Media ? 'bg-yellow-500 shadow-yellow-200' :
                  'bg-blue-500 shadow-blue-200'
                }`}>
                  <FiFileText size={24} />
               </div>
               <div className="flex-1">
                  {isEditing ? (
                    <input 
                      ...
                      onChange={(e) => setSelectedNota({...selectedNota, titulo: e.target.value})}
                    ...
                    <h2 className="text-2xl md:text-3xl font-black text-black dark:text-white uppercase tracking-tighter leading-none mb-2 pr-8">{selectedNota.titulo}</h2>
                  )}
                  
                   <div className="flex flex-wrap items-center gap-3 text-xs font-bold uppercase tracking-wider text-gray-400">
                      <span>{new Date(selectedNota.fechaCreacion).toLocaleDateString()}</span>
                      ...
                      {isEditing ? (
                        <>
                          <select 
                             ...
                             onChange={(e) => setSelectedNota({...selectedNota, categoria: Number(e.target.value)})}
                          ...
                          <select 
                             ...
                             onChange={(e) => setSelectedNota({...selectedNota, prioridad: Number(e.target.value)})}
                             className="bg-gray-100 dark:bg-gray-800 rounded p-1 text-xs font-bold"
                          >
                             <option value={NotaPrioridad.Baja}>BAJA</option>
                             <option value={NotaPrioridad.Media}>MEDIA</option>
                        ...
                        <>
                          ...
                          <span className="w-1 h-1 rounded-full bg-gray-300"></span>
                          <span className={`${
                            selectedNota.prioridad === NotaPrioridad.Alta ? 'text-red-500' :
                            selectedNota.prioridad === NotaPrioridad.Media ? 'text-yellow-500' :
                            'text-blue-500'
                          }`}>{NotaPrioridad[selectedNota.prioridad]}</span>
                        </>
                      )}
                   </div>
               </div>
            ...
            <div className="overflow-y-auto pr-4 custom-scrollbar -mr-4 flex-1">
               {isEditing ? (
                 <textarea 
                    ...
                    onChange={(e) => setSelectedNota({...selectedNota, contenido: e.target.value})}
